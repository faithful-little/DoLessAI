...
` }
    ...
    /**
     * Invoke smart scrape from within a workflow execution.
     * Takes a screenshot of the current page and runs the agentic scraper.
     * Returns the saved function name and extracted data.
     * 
     * @param {string} screenshotBase64 - Screenshot of current page state
     * @param {string} pageUrl - URL of the page being scraped
     * @param {string} apiKey - Gemini API key
     * @param {number} tabId - Tab ID for tool execution
     * @param {string} description - Description of what to extract
     * @param {function} onStatusUpdate - Status callback
     * @returns {object} { savedFunctionName, extractedData, success }
     */
    async invokeSmartScrapeForWorkflow(screenshotBase64, pageUrl, apiKey, tabId, description, onStatusUpdate) {
        try {
            onStatusUpdate(`SmartScrape: Analyzing page for "${description}"...`);

            const result = await this.agenticScrape(
                screenshotBase64,
                pageUrl,
                apiKey,
                tabId,
            ...
            if (result.savedFunction) {
                return {
                    success: true,
                    savedFunctionName: result.savedFunction.name,
                    extractedData: result.lastExtractionResult?.result || result.lastExtractionResult || [],
                    functionDef: result.savedFunction
                };
            } else {
                return {
                    success: false,
                    error: 'SmartScrape could not create extraction function',
                    extractedData: result.lastExtractionResult?.result || []
                };
            }
        } catch (e) {
            return {
    ...
    /**
     * Generate test URLs for validating scraper robustness.
     * Creates variations of the current URL to test extraction consistency.
     * 
     * @param {string} currentUrl - The current page URL
     * @param {string} apiKey - Gemini API key
     * @returns {Array} Array of { url, description } objects
     */
    async generateScraperTestUrls(currentUrl, apiKey) {
        const schema = {
            type: "OBJECT",
            properties: {
                testUrls: {
                    ...
                    items: {
                        type: "OBJECT",
                        properties: {
                            url: { type: "STRING", description: "Full test URL" },
        ...
        const prompt = `Given this URL: ${currentUrl}

Generate 2-3 test URLs to validate that a web scraper works consistently across different variations of this page.

RULES:
1. URLs must be REAL and VALID - no placeholders like "example" or "test"
2. For search pages: change the search query (e.g., "laptop" to "headphones")
3. For category pages: try different categories or filters
4. For listing pages: try different pagination (page 2, etc.)
5. Keep the same domain and page structure

Return URLs that would show SIMILAR DOM structures but DIFFERENT content.`;
        ...
        try {
            const result = await this.callGemini(requestBody, apiKey);
            return result.testUrls || [];
        } catch (e) {
            console.warn('Failed to generate test URLs:', e);
            // Fallback: return original URL only
            return [{ url: currentUrl, description: 'Original recording URL' }];
        }
    }
};
