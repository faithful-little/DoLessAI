document.addEventListener('DOMContentLoaded', () => {
    ...
    const backendEnabledCheckbox = document.getElementById('backendEnabled');
    const backendUrlInput = document.getElementById('backendUrl');
    const backendStatusEl = document.getElementById('backendStatus');
    const backendUploadEnabledCheckbox = document.getElementById('backendUploadEnabled');
    ...
    async function saveBackendSettings() {
        const payload = {
            ...
            backendEnabled: backendEnabledCheckbox?.checked === true,
            backendUrl: backendUrlInput?.value?.trim() || 'http://localhost:8787',
            backendUploadEnabled: backendUploadEnabledCheckbox?.checked === true
        };
        try {
            const response = await chrome.runtime.sendMessage(payload);
            if (!response?.success) {
                throw new Error(response?.error || 'Failed to save backend settings');
            }
            await checkBackendHealth();
        } catch (error) {
            console.warn('Failed to save backend settings:', error.message);
            if (backendStatusEl) {
                backendStatusEl.textContent = 'ERR';
                backendStatusEl.className = 'ollama-status offline';
                backendStatusEl.title = error.message;
            }
        }
    }
    ...
    async function checkBackendHealth() {
        ...
        backendStatusEl.className = 'ollama-status checking';
        try {
            ...
            if (response?.success) {
                backendStatusEl.textContent = 'ON';
                backendStatusEl.className = 'ollama-status online';
                const count = Number(response?.data?.functionCount || 0);
                backendStatusEl.title = `Online (${count} functions)`;
            } else {
                backendStatusEl.textContent = 'OFF';
                backendStatusEl.className = 'ollama-status offline';
                backendStatusEl.title = response?.error || 'Backend unavailable';
            }
        } catch (error) {
            backendStatusEl.textContent = 'ERR';
            backendStatusEl.className = 'ollama-status offline';
    ...
    backendEnabledCheckbox?.addEventListener('change', saveBackendSettings);
    backendUploadEnabledCheckbox?.addEventListener('change', saveBackendSettings);
    backendUrlInput?.addEventListener('change', saveBackendSettings);
    ...
    // Load saved settings
    chrome.storage.local.get(['selectedAudioDeviceId', 'geminiApiKey', 'aiRetryCount', 'aiThinkingLevel', 'generatedFunctions', 'showTestsForeground', 'computerUseMaxActions', 'enableProactiveExploration', 'backendEnabled', 'backendUrl', 'backendUploadEnabled'], (data) => {
        ...
        if (backendEnabledCheckbox) {
            backendEnabledCheckbox.checked = data.backendEnabled === true;
        }
        if (backendUrlInput && typeof data.backendUrl === 'string' && data.backendUrl.trim()) {
            backendUrlInput.value = data.backendUrl.trim();
        }
        if (backendUploadEnabledCheckbox) {
            backendUploadEnabledCheckbox.checked = data.backendUploadEnabled === true;
        }
        checkBackendHealth();
        updateGenerateButtonState();
    });
    ...
    function buildRecordingBackendQuery(recording = { steps: [] }) {
        ...
        for (const step of steps.slice(0, 20)) {
            ...
            const target = step?.elementName ? String(step.elementName) : '';
            const selector = step?.selector ? String(step.selector) : '';
            const value = step?.value ? String(step.value) : '';
            const text = `${action} ${target} ${selector} ${value}`.trim();
            if (text) parts.push(text);
        }
        return parts.join(' ').trim();
    }

    async function hydrateBackendFunctionsForContext(query, currentUrl = '', options = {}) {
        const reasonLabel = options.reason || 'backend context';
        const onlyWhenNoLocalMatches = options.onlyWhenNoLocalMatches !== false;
        const topK = Number.isFinite(Number(options.topK)) ? Number(options.topK) : 8;

        try {
            const response = await chrome.runtime.sendMessage({
                type: 'hydrateBackendFunctionsForTask',
                query: String(query || '').trim(),
                currentUrl: String(currentUrl || '').trim(),
                onlyWhenNoLocalMatches,
                topK
            });
            if (!response?.success || !response?.result) {
                return { success: false, importedCount: 0, error: response?.error || 'backend-hydration-failed' };
            }
            const importedCount = Number(response.result.importedCount || 0);
            if (importedCount > 0) {
                allFunctions = await loadFunctionLibrary();
                updateFunctionsLibrary();
                updateAITaskFunctionsLibrary();
                updateReferenceFunctionsList();
                addLogEntry(`☁️ Loaded ${importedCount} backend function(s) for ${reasonLabel}`);
            }
            return {
                success: true,
                importedCount,
                skipped: response.result.skipped === true,
    ...
    async function generateFunctionFromRecording() {
        ...
        const startUrl = currentRecordingState.steps?.[0]?.url || '';
        const localRelatedBeforeBackend = getRelatedFunctions(startUrl);
        if (startUrl && localRelatedBeforeBackend.length === 0) {
            const backendQuery = buildRecordingBackendQuery(currentRecordingState);
            await hydrateBackendFunctionsForContext(backendQuery, startUrl, {
                onlyWhenNoLocalMatches: true,
                reason: 'recording generation',
                topK: 8
            });
        }
    ...
    aiTaskGenerateBtn.addEventListener('click', async () => {
        ...
        await hydrateBackendFunctionsForContext(taskDescription, currentPageUrl, {
            onlyWhenNoLocalMatches: true,
            reason: 'AI task planning',
            topK: 10
        });
            ...
            if (toolDetection.needsTools) {
                ...
                try {
                    ...
                    if (result.success && result.result) {
                        ...
                        const bestEffort = chainResult.bestEffort === true;
                        showAITaskStatus(
                            bestEffort
                                ? `Tool chain completed with estimates: ${chainResult.totalSteps} steps (${chainResult.failedSteps} failed)`
                                : `Tool chain completed: ${chainResult.totalSteps} steps (${chainResult.failedSteps} failed)`,
                            bestEffort ? 'active' : (chainResult.failedSteps === 0 ? 'success' : 'active')
                        );
                        addLogEntry(`✅ Tool chain completed. Steps: ${chainResult.totalSteps}, Failed: ${chainResult.failedSteps}`);

                        if (bestEffort) {
                            addLogEntry('Best-effort mode: returned estimated comparison output because repair was skipped near timeout.');
                        }
