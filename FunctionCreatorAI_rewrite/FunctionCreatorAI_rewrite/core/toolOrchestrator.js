...
For the API key, use {{apiKey}}.`;
    ...
    async _captureTabScreenshot(tabId) {
        ...
        try {
            ...
            // Allow newly-opened report tabs (e.g., viewer.html) to finish painting before capture.
    ...
    _extractPrimaryOutputText(notepadState = {}, steps = []) {
        ...
        for (const step of [...successfulSteps].reverse()) {
            ...
            const outputPayload = (
                step?.result && typeof step.result === 'object' && step.result.outputPreview !== undefined
                    ? step.result.outputPreview
                    : step?.result
            );
            const text = this._stringifyForVerification(outputPayload).trim();
            if (!text) continue;
            if (looksMetadataOnly(outputPayload, text)) continue;
...
- For cross-site comparison tasks, mixed currencies are acceptable only if the output clearly preserves original prices and includes a normalized estimate (for example, USD estimate) with an approximation disclaimer.
...
Entries: ${JSON.stringify(sentimentBatchInput.map(({ index, text }) => ({ index, text })))}`;
    ...
    async executeFullPipeline(taskDescription, apiKey, options = {}) {
        ...
        let repairAttempt = 0;
        let repairSkippedForTimeout = false;
        const pipelineStartMs = Date.now();
        const repairSoftDeadlineMs = 260000;
        const comparisonLikeTask = /\b(compare|comparison|versus|vs\.?|side[-\s]?by[-\s]?side)\b/i.test(String(taskDescription || ''));
        const hasUsableOutput = (executionResult) => {
            const primaryText = this._extractPrimaryOutputText(executionResult?.notepadState, executionResult?.steps).text.trim();
            if (primaryText.length > 0) return true;
            const outputTools = new Set(['webpage_generator', 'file_system_maker', 'site_modifier']);
            return Array.isArray(executionResult?.steps)
                && executionResult.steps.some(step => step?.success && outputTools.has(step.tool));
        };

        while (result.success || (strictVerification && hasUsableOutput(result))) {
            if (shouldAbort()) {
                throw new Error('Stopped by user');
            }
            ...
            if (elapsedMs >= repairSoftDeadlineMs) {
                repairSkippedForTimeout = true;
                onStatusUpdate(
                    `Auto-repair skipped to avoid timeout (${Math.round(elapsedMs / 1000)}s elapsed). Returning best-effort output.`,
                    {
                        type: 'tool-chain-repair-skipped',
                        reason: 'timeout'
                    }
                );
                break;
            ...
            if (!repairedResult.success) {
                onStatusUpdate('Auto-repair execution had failed steps. Verifying best available output...', {
                    type: 'tool-chain-repair-complete',
                    success: false,
                    partial: true
                });
        ...
        const issueBlob = (verification?.issues || []).join(' ').toLowerCase();
        const estimateFriendlyFailure = /currency|regional mismatch|ui quality score|no screenshots|missing final ui/.test(issueBlob);
        const hasPrimaryOutput = this._extractPrimaryOutputText(result?.notepadState, result?.steps).text.trim().length > 0;
        const bestEffortFallbackUsed =
            comparisonLikeTask &&
            repairSkippedForTimeout &&
            result.success &&
            hasPrimaryOutput &&
            estimateFriendlyFailure;
        const finalSuccess =
            bestEffortFallbackUsed ||
            (strictVerification && !result.success && verificationPassed && hasPrimaryOutput) ||
            (result.success && (!strictVerification || verificationPassed));

        let savedFunction = null;
        if (finalSuccess && !bestEffortFallbackUsed && result.success) {
            if (shouldAbort()) {
                throw new Error('Stopped by user');
            }
            try {
        ...
        } else if (bestEffortFallbackUsed) {
            onStatusUpdate('Returned best-effort estimated comparison output (verification not fully passed).', {
                type: 'tool-chain-best-effort',
                reason: 'timeout_before_repair'
            });
        } else if (finalSuccess && !result.success) {
            onStatusUpdate('Returned verified output despite some failed repair steps. Function was not auto-saved.', {
                type: 'tool-chain-best-effort',
                reason: 'partial_steps_verified'
            });
        } else {
        ...
        return {
            ...
            verification,
            bestEffort: bestEffortFallbackUsed,
            savedFunction
        };
    },
