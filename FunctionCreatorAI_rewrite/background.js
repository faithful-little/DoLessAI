...
async function findReusableSmartScraper(currentUrl, taskDescription) {
    ...
    const urlMatched = generatedFunctions.filter(func => {
        if (func.source !== 'smartScrape') return false;
...
async function getReturnValue(tabId, selector) { const [res] = await chrome.scripting.executeScript({ target: { tabId }, func: (sel) => { const els = Array.from(document.querySelectorAll(sel)); if (els.length > 0) return els[els.length - 1].innerText.trim(); return null; }, args: [selector] }); return res?.result; }
...
function waitForUrl(tabId, targetPattern, timeout = 15000) { return new Promise((resolve, reject) => { const start = Date.now(); const i = setInterval(async () => { if (Date.now() - start > timeout) { clearInterval(i); reject(new Error(`Timed out for URL: ${targetPattern}`)); return; } try { const tab = await chrome.tabs.get(tabId); if (tab && tab.status === 'complete' && tab.url && urlMatchesPattern(tab.url, targetPattern)) { clearInterval(i); resolve(); } } catch (e) { clearInterval(i); reject(e); } }, 500); }); }
...
function waitForTextToSettle(tabId, selector, timeout = 45000) { return new Promise(async (resolve) => { const [res] = await chrome.scripting.executeScript({ target: { tabId }, func: (sel, to) => new Promise(r => { const startTime = Date.now(); let lastText = ''; let stableTime = Date.now(); const interval = setInterval(() => { if (Date.now() - startTime > to) { clearInterval(interval); r(false); return; } const els = Array.from(document.querySelectorAll(sel)); if (els.length > 0) { const currentText = els[els.length - 1].innerText; if (currentText === lastText) { if (Date.now() - stableTime > 1500) { clearInterval(interval); r(true); } } else { lastText = currentText; stableTime = Date.now(); } } }, 500); }), args: [selector, timeout] }); resolve(res?.result); }); }
...
async function executeAIExecuteFunction(data, tabId) {
    ...
    const functionDef = await FunctionLibraryService.get(name);
...
async function executeAICountElements(data, tabId) {
    // Count elements matching selector - used for scroll-for-more content detection
    try {
        ...
        return result?.result || { success: true, count: 0 };
    } catch (e) {
        return { success: false, error: e.message, count: 0 };
    }
}
